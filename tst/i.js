        ...
        window.showInitialActivationDocument = function ()
{
	...
	if(document_details != null)
	{
		...
		if(document_details != null && document_details.id_front != undefined && document_details.id_front != null && document_details.id_front !="")
		{
			...
			if (img) {
  				const clone = img.cloneNode(true);       // clone complet (attributs + enfants)
  				clone.id = img.id + '2';                 // documentImageId -> documentImageId2
				const clone2 = img.cloneNode(true);       // clone complet (attributs + enfants)
  				clone2.id = img.id + '3';                 // documentImageId -> documentImageId2
	
  				img.parentNode.insertBefore(clone2, img.nextSibling); // insère juste après l'original
				img.parentNode.insertBefore(clone, img.nextSibling); // insère juste après l'original
			}
        ...
        console.log('[Extension] doSomething overriden');
      } else {
        // si la fonction n'est pas encore définie, réessayer
        console.log('[Extension] doSomething non trouvée, réessai...');
...
// Appelle la fonction pour les deux champs
